...
def get_hit_classification_dataset(split=False, backtest_date=''):
    ...
    speed_df = query_mlb_db(
            'select mlb_id as batter, speed from PlayerSpeed;')
    venue_df = query_mlb_db(
            'select game_pk, venue_name from VenueGamePkMapping;')
    oaa_df = query_mlb_db("""
        select o.year as 'game_year', t.STATCAST as 'fielding_team',
                          o.oaa_rhh_standardized, o.oaa_lhh_standardized
        from TeamOAA o
    ...
    df = df.drop(['batter', 'game_pk', 'oaa_rhh_standardized',
                  'oaa_lhh_standardized'], axis=1)

    target_col = 'simplified_outcome'

    return df, target_col #get_xgb_set(df, target_col=target_col, split=split)
...
def get_sequencing_dataset(pitcher, backtest_date=''):
    ...
    query_str = f"""
        ...
        WHERE pitcher = {pitcher}
            AND pitch_type IS NOT NULL
            and pitch_type <> 'PO'
            AND game_type <> 'E' || 'S' /* avoid games with experimentation*/
        ORDER BY game_year, at_bat_number, pitch_number
    """
    pitcher_df = query_mlb_db(query_str).set_index('batter')
