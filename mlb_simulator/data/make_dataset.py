...
def validate_db_and_table(table_name):
    if not Path(DB_LOCATION).exists():
        logger.critical(
            f"""mlb.db doesnt exist at {DB_LOCATION},
...
def update_statcast_table():
    ...
    if not statcast_table_exists:
        ...
        try:
            with open(THIS_FILEPATH / "sql_scripts" / "Statcast_create.sql", "r") as f:
    ...
    # otherwise, get all data from max date in table
        ...
        max_date = pd.read_sql("select max(game_date) from Statcast", engine)[
            "max(game_date)"
        ].iloc[0]
            ...
            for year in range(max_date.year, today.year + 1):
                ...
                if not df.empty:
                    ...
                    if year == max_date.year:
                        ...
                        upload_df = merged.loc[merged["_merge"] == "left_only"].drop(
                            "_merge", axis=1
                        )
                    else:
                        upload_df = df
...
def update_woba_strike_tables(min_pitch_count=50, min_hit_count=15, backtest_yr=None):
    ...
    strike_percentage = filtered_data.groupby(["batter", "pitch_type"])["type"].apply(
        lambda x: (x == "S").mean() * 100
    )
    strike_percentage_standardized = (
        strike_percentage.groupby("pitch_type").transform(zscore).reset_index()
    )

    strike_df = strike_percentage_standardized.pivot(
        index="batter", columns="pitch_type", values="type"
    ).fillna(0)
    ...
    """
    filtered_data = (
        batter_df[batter_df["type"] == "X"]
        .groupby(["batter", "pitch_type"])
        .filter(lambda x: (x["type"] == "X").sum() >= min_hit_count)
    )

    average_woba = filtered_data.groupby(["batter", "pitch_type"])["woba_value"].mean()
    average_woba_standardized = (
        average_woba.groupby("pitch_type").transform(zscore).reset_index()
    )
...
def update_chadwick_repo():
    ...
    repo_save_path = os.path.join(
        home_dir, "sports", "mlb_simulator", "data", "raw", "chadwick"
    )
...
def update_venue_game_pk_mapping():
    ...
    if not venue_table_exists:
        logger.info("Running creation/init upload for VenueGamePkMapping table")
        ...
        try:
            ...
            init_upload_df.to_sql(
                "VenueGamePkMapping", engine, if_exists="replace", index=False
            )
    ...
    missing_game_pks = game_pks[~game_pks["game_pk"].isin(venue_game_pks["game_pk"])][
        "game_pk"
    ].to_list()
    ...
    if missing_game_pks:
        ...
        upload_df = df.rename(columns={"name": "venue_name", "id": "venue_id"})[
...
def update_oaa():
    ...
    for col in ("outs_above_average_rhh", "outs_above_average_lhh"):
        df[f"{col}_standardized"] = df.apply(
            lambda row: (row[col] - means.loc[row["year"]][col])
            / stds.loc[row["year"]][col],
            axis=1,
        )
...
def main():
    ...
    if not os.path.exists(name_map_path):
        logger.info(f"Creating raw data folder with player map at {name_map_path}")
