"""
The schedule object will scrape daily projected lineups from rotowire. Contains
methods to work with the Lineup object.
"""
    ...
    def _get(self):
        ...
        if not self.for_today:
            res = requests.get(Schedule.ROTOWIRE_URL + \
                    Schedule.ROTOWIRE_TOMORROW_SUFFIX)
        else:
            res = requests.get(Schedule.ROTOWIRE_URL)

        if res.status_code == 200:
            game_times, game_soups = self._process_res(res)
            self._games = [Game(game_times[i], game_soups[i])
