...
class Batter(Player):
    def __init__(self, mlb_id=None, rotowire_id=None, backtest_date=''):
        ...
        self.stand = query_mlb_db(
            f'select stand from Statcast where batter={
                self.mlb_id} and stand is not null limit 1')['stand'][0]
        self.pitch_woba = dict(
            query_mlb_db(
                f'select * from BatterAvgWobaByPitchType where batter={
                    self.mlb_id};').drop(
                'batter',
                axis=1).iloc[0])
        self.pitch_strike = dict(
            query_mlb_db(
                f'select * from BatterStrikePctByPitchType where batter={
                    self.mlb_id};').drop(
                'batter',
                axis=1).iloc[0])
        self.speed = float(
            query_mlb_db(
                f'select speed from PlayerSpeed where mlb_id={
                    self.mlb_id}')['speed'][0])
        self.sz = dict(
            query_mlb_db(
                f'select sz_bot, sz_top from BatterStrikezoneLookup where batter={
                    self.mlb_id}').iloc[0])
    ...
    def fit_pitch_outcome_model(self):
        X, y, encoders = get_pitch_outcome_dataset_xgb(
            self.mlb_id, backtest_date=self.backtest_date)
        ...
        self.pitch_outcome_clf = xgb.XGBClassifier(
            objective='multi:softprob', eval_metric='mlogloss', **params)
        self.pitch_outcome_clf.fit(X, y)

        assert (
            compute_xgboost_loglik(
                self.pitch_outcome_clf.predict_proba(X),
                y,
                'pitch_outcome') > compute_cat_loglik(
                X,
                y,
                y,
                'pitch_outcome'))
    ...
    def generate_pitch_outcome(self, game_state, pitch_characteristics):
        ...
        outcome_choice_encoded = np.random.choice(
            len(outcome_distribution), size=1, p=outcome_distribution)
        outcome_choice = self.pitch_outcome_encoders['pitch_outcome'].inverse_transform(
            outcome_choice_encoded)[0]
        return outcome_choice

    # predicts hit launch speed, angle, etc.

    def fit_hit_outcome_model(self):
        X, y, encoders = get_hit_outcome_dataset(
            self.mlb_id, backtest_date=self.backtest_date)
        ...
        for column in self.hit_outcome_cols:
            ...
            cdf_vals = np.array([kde.integrate_box_1d(-np.inf, x)
                                for x in x_vals])
            inverse_cdf = interp1d(
                cdf_vals, x_vals, bounds_error=False, fill_value=(x_vals[0], x_vals[-1]))
        ...
        for i, column in enumerate(y.columns):
            cdf_values = np.array(
                [kde_estimators[i].integrate_box_1d(-np.inf, x) for x in y[column]])
        ...
        # Ensure no infinite values (can occur if uniform marginals are exactly
        # 0 or 1)
    ...
    def generate_hit_outcome(self):
        ...
        # Step 5: Transform new samples from standard normal to uniform
        # marginals
        ...
        generated_dataframe = pd.DataFrame(
            new_samples, columns=self.hit_outcome_cols)
